...
Dashboard 的分析工作台可輸入 Docker 服務名稱或單一私人網路 IP。系統會拒絕公開網際網路 IP，避免將教學掃描器用於未授權目標。輸入目前 Lab Target 的服務名稱或動態 IP 時，會額外啟用授權 UNIX Collector 與受控攻擊展示；其他私人主機只執行 Nmap 與 HTTP 探測。

## 獨立 Web Demo

專案另提供一個沒有預設漏洞的 Linux Web 服務，用來測試一般網站掃描：
...
系統會以黑箱方式掃描這個服務，通常會發現 `8080/tcp http`，但不會出現 Target 專用的受控攻擊漏洞。
